...
export async function migrate(databaseUrl: string): Promise<void> {
  ...
  try {
    ...
    `;
    await revokeLedgerWriteAccess(sql, 'schema_migrations');
    ...
    const applied = await sql<{ version: string; checksum: string | null }[]>`
      ...
      ORDER BY version
    `;
    assertMigrationLedgerIntegrity(migrations, applied, 'schema_migrations');
    await sql`
      ALTER TABLE schema_migrations
...
export function readMigrationDatabaseUrl(environment: NodeJS.ProcessEnv = process.env): string {
  ...
  const value = environment['DATABASE_MIGRATION_URL']?.trim();
  if (value === undefined || value === '') {
    throw new Error(
      'DATABASE_MIGRATION_URL is required for the explicit indexer migration command.',
    );
  }
  let url: URL;
  try {
