...
export class FailoverSolanaRpc implements FinalizedSolanaRpc {
  ...
  constructor(
    ...
    if (endpoints.length === 0) {
      throw new SolanaRpcValidationError('At least one Solana RPC endpoint is required.');
    }
  }
  ...
  async #execute<T>(
    ...
    throw new SolanaRpcUnavailableError(`All Solana RPC endpoints failed. ${failures.join('; ')}`);
  }
}
