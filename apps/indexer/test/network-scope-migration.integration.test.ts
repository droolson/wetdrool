...
describe('0008 and 0009 network-scope migrations', () => {
  it('upgrade a populated 0007 schema and reject cross-network references', async () => {
    ...
    const networkOne = `woke:v1:${publicKey()}:${programId}`;
    const networkTwo = `woke:v1:${publicKey()}:${programId}`;
    const identityAddress = publicKey();
    const subjectIdentityAddress = publicKey();
    const identityOne = `swid:v1:${networkOne}:${identityAddress}`;
    const identityTwo = `swid:v1:${networkTwo}:${identityAddress}`;
    const subjectIdentityOne = `swid:v1:${networkOne}:${subjectIdentityAddress}`;
    const subjectIdentityTwo = `swid:v1:${networkTwo}:${subjectIdentityAddress}`;
    ...
    try {
      ...
      const networkThree = `woke:v1:${publicKey()}:${programId}`;
      const identityThreeAddress = publicKey();
      const subjectIdentityThreeAddress = publicKey();
      const unusedIdentityOneAddress = publicKey();
      const identityThree = `swid:v1:${networkThree}:${identityThreeAddress}`;
      const subjectIdentityThree = `swid:v1:${networkThree}:${subjectIdentityThreeAddress}`;
      const unusedIdentityOne = `swid:v1:${networkOne}:${unusedIdentityOneAddress}`;
      await sql`
        INSERT INTO identities (
          identity_id, network_id, identity_address, root_authority,
          root_rotation_count, created_slot, created_at, updated_slot, updated_at
      ...
      await expect(
        sql`
          ...
          VALUES (
            ${`swid:v1:${networkOne}:${mismatchedIdentityAddress}`},
