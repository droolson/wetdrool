# WetDrool for Solana Mobile

This Expo/React Native Android app is the WetDrool client foundation for
Solana Seeker and other Mobile Wallet Adapter-compatible Android devices.
DroolNet is the WetDrool program/deployment namespace on Solana; this app does
not connect to a separate DroolNet blockchain or a DroolNet
...
- Explicit wallet connect/disconnect. A connected wallet is a signer, not by
  itself a WetDrool identity.
- Finalized verification of the configured Solana genesis and executable
  WetDrool program account before protocol features can be enabled.
...
does not yet select a WetDrool identity, sign the membership manifest,
...
devnet/mainnet WetDrool program ID is recorded in this repository, and the
legacy lamport-as-WOKE payment ABI is disabled. `$DROOL` requires a separately
...
```text
droolnet:v1:<Solana genesis hash>:<WetDrool program ID>
```
...
```sh
pnpm --filter @wetdrool/mobile android
pnpm --filter @wetdrool/mobile dev
```
...
```sh
pnpm --filter @wetdrool/mobile lint
pnpm --filter @wetdrool/mobile typecheck
pnpm --filter @wetdrool/mobile test
pnpm --filter @wetdrool/mobile build
```
...
```sh
pnpm --filter @wetdrool/mobile build:apk
```
