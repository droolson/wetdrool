// Rebuilt from the droolnet repository docs (docs/*.md), condensed for the web.
export const DOCS = [
{slug:'readme',cat:'Product & experience',title:'Documentation index',file:'README.md',status:'Reference',purpose:'How the WetDrool and DroolNet documentation set is organised, and the vocabulary every document shares.',sections:[
{h:'What this documentation covers',blocks:[
{t:'p',x:'This set contains the product, protocol, trust, safety, development, and operations documentation for WetDrool and DroolNet. WetDrool is the social platform and flagship product at wetdrool.com. DroolNet is its portable protocol and Anchor smart-contract layer on Solana — not a blockchain, validator network, Solana fork, or Firedancer/Agave topology.'},
...
{t:'ul',items:['WetDrool is the platform; DroolNet is the protocol and Solana program layer; Solana is the underlying chain.','Local-validator results are never described as devnet, mainnet-beta, or production evidence.','Legacy hostnames stay redirect-only — the canonical origin is wetdrool.com.','Every compatibility-significant decision gets an architecture decision record before merge.']}]}]},

{slug:'product-spec',cat:'Product & experience',title:'Product specification',file:'PRODUCT_SPEC.md',status:'Requirements baseline',purpose:'Product principles, audiences, journeys, feature boundaries, and delivery status for the flagship WetDrool client.',sections:[
{h:'Product vision',blocks:[
{t:'p',x:'WetDrool is a polished, inclusive social platform for everyone, powered by DroolNet on Solana. People own and move their identity, social graph, public content relationships, communities, and choice of algorithm without needing to understand cryptocurrency. The flagship client must feel like a trustworthy consumer application rather than a wallet or blockchain explorer.'},
{t:'p',x:'A person may browse without a token, create a recoverable passkey-first account, use a familiar handle, and understand every transaction before approving it. The approved expansion adds decentralized video, pseudonymous .drool names, contribution points, an avatar and creator-item ecosystem, optional minimal-disclosure verification, open-model AI tools, market research, and published governance.'}]},
...
{t:'p',x:'Chosen display names are independent of legal names, which are never requested in ordinary onboarding. Sensitive profile fields are private by default, never inferred, and excluded from recommendations. Human-readable ASCII v1 handles are globally collision-safe onchain and distinct from wallet addresses; versioned anonymous .drool candidates derive deterministically from the identity origin.'},
{t:'callout',x:'A payment or signature flow must reveal the current Solana destination, network, deployed DroolNet program, fees and rent. A .drool name is never rendered as a native wallet address, and a handle alone never implies legal identity.'}]}]},

{slug:'platform-expansion',cat:'Product & experience',title:'Platform expansion',file:'PLATFORM_EXPANSION.md',status:'Approved direction',purpose:'The eight connected workstreams that turn the expanded wetdrool.com vision into testable product systems.',sections:[
{h:'The eight workstreams',blocks:[
{t:'ul',items:['Decentralized long-form and short-form video','Pseudonymous .drool names','Contribution points and AI credits','An avatar studio and creator-item marketplace','Minimal-disclosure account verification and age assurance','An open-model assistant and multimodal creator suite','Social-sentiment market intelligence and bounded automation','Open protocol and product-rule governance']},
...
{h:'.drool names',blocks:[
{t:'p',x:'A .drool value is a DroolNet name that resolves to a real Solana public key. New accounts derive a deterministic anonymous handle — anon_ plus 16 Crockford-base32 characters of a domain-separated SHA-256 prefix over the identity origin — with no server randomness, stable across recovery and rotation, and protected onchain against front-running.'},
...
{t:'callout',x:'Highly constructive people must have a realistic non-payment path to every ordinary platform benefit. Any future conversion of points to SOL or $DROOL is a separate accounting boundary with its own reserve model, fraud controls, tax and consumer-protection analysis, and release approval.'}]},
...
{t:'p',x:'Protocol changes, name rules, point formulas, marketplace policy, AI model policy and automation limits are published and versioned. Material changes require a public proposal, exact diffs, impact analysis, conflict-of-interest disclosure, security review, delayed activation and rollback rules. SOL, NFTs, purchased points, or a future $DROOL balance do not automatically control governance.'},
...
{slug:'ai-platform',cat:'Product & experience',title:'Drool AI platform',file:'AI_PLATFORM.md',status:'Approved direction',purpose:'Athena, Kairos, Hermes — the model family, owned-hardware evaluation, runtime contract, credits, training rules, and release gates.',sections:[
...
['Drool Hermes','Fastest and least expensive, strong tool discipline','Lowest','Classification, extraction, routing, background agents, repetitive creator tasks']]},
...
['Woke Social, Inc.','Subsidiary corporation','wetdrool.com application, WetDrool services, DroolNet protocol stewardship, community products'],
['Drool AI, Inc.','Delaware C-corp subsidiary','Drool AI model development, inference, evaluation, planned pinkman.ai services'],
...
{t:'p',x:'An ownership relationship does not erase service boundaries. WetDrool and Drool AI are separate service and data boundaries even under one parent. Each cross-entity data flow requires a named purpose, responsibility decision, least-privilege access, retention rule, security obligations, user-facing disclosure, an auditable path, and an exit plan. ICEFAM Records receives no WetDrool user data or Drool AI prompts by default.'}]},
{h:'Open-source and governance boundary',blocks:[
{t:'ul',items:['DroolNet specifications and first-party source remain under the published open-source license.','A corporation may operate the flagship client without becoming the only valid client, indexer, storage, feed, RPC or AI provider.','Purchased points, SOL, NFTs, subscriptions, shares, or a future $DROOL balance do not automatically grant protocol governance power.']}]},
{h:'Public naming',blocks:[
{t:'p',x:'WetDrool for the product; DroolNet for the Solana protocol namespace; Drool AI for the model and service family; Drool Athena, Drool Kairos and Drool Hermes for measured model tiers; Pinkman, Inc. only for the umbrella organization.'}]}]},
...
{h:'Voice',blocks:[
{t:'p',x:'Plainspoken, warm, and precise — and quieter as stakes rise. Second person for the user; first-person plural only when WetDrool is acting or accountable. Sentence case everywhere. Outcomes over infrastructure: "Post", never "Submit transaction". Nothing claims to be confirmed until it is confirmed.'},
...
{t:'p',x:'The repository has a verified local foundation and experimental subsets: pinned toolchains with healthy PostgreSQL/Redis/Kubo services; an SBF-built DroolNet program subset tested against a disposable Solana local validator; TypeScript protocol, storage, SDK and indexer subsets; a finalized RPC synchronizer with a green one-command connected slice from fresh validator through signed content-addressed storage, projection replay, production API and web; signed relay and moderation-provider subsets; 210 passing browser cases and 90 axe checks; and a non-release Expo/React Native Seeker Android foundation.'},
...
['2','DroolNet identity, profiles, delegation, follows, communities','Experimental local program subset'],
...
['9','Future $DROOL tips, subscriptions, entitlements','No mint; legacy lamport ABI quarantined'],
...
{slug:'protocol',cat:'Architecture & protocol',title:'Protocol',file:'PROTOCOL.md',status:'Draft v0.1 · experimental subset',purpose:'Canonical objects, signatures, identifiers, program accounts, instructions, events, and compatibility rules for DroolNet on Solana.',sections:[
...
{t:'code',x:'droolnet:v1:<base58-genesis-hash>:<base58-program-id>\nwetdroolid:v1:droolnet:v1:<genesis>:<program>:<identity-pda>\nid = wetdroolobj:v1:<object-type>:<base64url(SHA-256(JCS(payload)))>'},
{t:'p',x:'A DroolNet deployment is identified by the Solana genesis hash plus program ID — clients compare both before reading or signing. The identity PDA stays stable when its root authority rotates; wallet addresses and delegated keys are attributes of the identity, not its display name. Development-localnet program ID: 9kFGJEzA7uKvJ1wTvKRWoFadRU7WFnpwWEGP6APro3dD.'}]},
...
{t:'p',x:'Every component is classified as canonical or convenience, with its current operator, replaceability, failure behaviour, censorship and privacy risk, and migration procedure. Canonical: the Solana ledger, the DroolNet program, identity roots and delegations, signed public manifests, community commitments. Convenience: RPC providers, indexers, PostgreSQL projections, relays, feed providers, moderation-label providers, media processors, search, sponsorship, passkey assistance, and both flagship clients.'},
{t:'ul',items:['Only finalized DroolNet program facts and valid signed objects establish public protocol claims.','Convenience services can select, order, label, transport or transform data — but cannot silently create protocol truth.','A user or community export bundle must be consumable without contacting any wetdrool.com API.']}]},
...
{t:'ul',items:['Solana validator concentration and popular RPC reliance are external dependencies.','Any future $DROOL mint adds mint authority, token-program, liquidity, custody, regulatory and integration risks.','Popular gateways, clients and moderation providers can become de facto central points.','Content addressing provides integrity, not persistence; strong privacy is hard when public relationships are onchain.']}]}]},
...
['0007','Messaging cryptographic engine'],['0008','Canonical-domain transition to wetdrool.com'],
['0009','DroolNet as a Solana program and protocol layer'],['0010','Verified community discovery'],
['0011','Member-signed community membership'],['0012','Versioned .drool identity names']]}]},
{h:'ADR-0009 — DroolNet on Solana',blocks:[
{t:'p',x:'DroolNet is a Solana program and protocol layer, not a separate chain, fork, validator implementation or RPC network. Solana supplies the external ledger, finality and runtime; DroolNet supplies identity, authorization, references, tombstones and community facts. Human labels like localnet or mainnet-beta are display hints — the cryptographic deployment identity is the genesis hash plus program ID.'}]},
{h:'ADR-0012 — versioned .drool names',blocks:[
