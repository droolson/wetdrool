...
This package is the portable, implementation-independent WetDrool signed-object layer. It provides strict v1 Zod schemas, typed builders, canonical serialization, stable object IDs, Ed25519 envelopes, authorization requirements, and immutable transition validation.

The package validates claims. It does not turn a signed claim into authoritative network state. Indexers and clients must still reconcile the claim with finalized DroolNet state, community policy, and content-addressed storage.

The distributable Draft 2020-12 signed-envelope schema is available at
`@wetdrool/protocol/schema/v1` and checked in under
`schemas/wetdrool-signed-envelope-v1.schema.json`. `schema:check` derives
...
- a canonical DroolNet ID,
  `droolnet:v1:<genesis-hash-base58-32>:<program-id-base58-32>`
...
```text
wetdroolobj:v1:<type>:<multibase-base64url-sha256(canonical-payload)>
```
...
current protected shape. The canonical DroolNet namespace has a pinned v1 post
...
```ts
{
  protection: {
    ...
    keyEnvelope: { id: 'wetdroolobj:v1:...' },
    accessPolicy: { id: 'wetdroolobj:v1:...' }
  }
}
```
...
- an optional DroolNet treasury account using the Solana-compatible 32-byte public-key representation, policy reference, and bounded asset allow-list.
...
community account address to one network-local DroolNet identity. The first
...
- canonical identity IDs:
  `wetdroolid:v1:<network-id>:<identity-address-base58-32>`;
...
`INDEXER_PROFILE_V2_ACTIVATION_SLOT` per DroolNet: a historical profile-update
event without the appended commitment may reference readable legacy v1 only
before that slot. Current root and delegated WetDrool instructions accept
