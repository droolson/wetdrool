...
| `apps/indexer` | Finalized Solana RPC and content ingestion, PostgreSQL projections, public query API | Database is disposable and noncanonical | Finalized live synchronization, exhaustive decoding/projection of all 33 current-IDL events including identity deactivation and member-signed membership, schema-aware profile/community/membership manifest verification, exact CID/URI validation, checkpoint-independent bounded hydration, suppression-aware memory/PostgreSQL rebuild, 18 ordered migrations, explicit accepted/pending/terminal manifest state, privacy-safe bounded community directory/detail, exact-address membership status without roster/identity fields, and `public-match-v2` search are implemented. The consumer-safe home response remains separate from `/v1/feed`; fork/reorg, independent-provider, and production-scale rebuild evidence remain blocked |
...
ban is terminal. Governance snapshots the community membership sequence and
requires a voter to have become active no later than that snapshot.
...
The current TypeScript implementation covers all 29 registered portable object
families. Profile, community, and community-membership use schema v2 for
current creation with frozen v1 read compatibility. It also covers RFC 8785
canonicalization, SHA-256 identifiers, Ed25519
...
Member-signed membership events verify the exact portable transition,
member/moderator author, deterministic PDA, action/state/roles, and optimistic
sequences. The public API offers only exact membership-address status for
verified open public/unlisted communities after a covering checkpoint. It has
no roster and omits member/actor identities, signer authority, moderation
