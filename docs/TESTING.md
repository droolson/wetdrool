...
Solana, not a chain or validator network. No `$DROOL` mint exists. The legacy
lamport payment ABI is quarantined and cannot execute or be unpaused; its tests
are regression evidence only.
No Firedancer/Agave validator topology is part of DroolNet.
...
ephemeral loopback port. It proves exact legacy `droolhouse.com` and
`www.droolhouse.com` `Host` values return a path/query-preserving `308` to
`https://wetdrool.com` and proves the canonical, suffix-lookalike, and
...
Adapter, signed-APK, or `$DROOL`-mint behavior.
