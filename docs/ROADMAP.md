...
| `.woke` custom-name settlement | ADR-0012 now fixes deterministic anonymous allocation and program-enforced `anon_` anti-front-running. Atomic registration/claim, independently verifiable current-destination UX, reserved-name governance, commit/reveal, expiry/cooldown/transfer/recovery rules, disputes, and economics review remain required |
...
| Social-sentiment automation | Sourced signals, manipulation filters, paper-trading evidence, client-held keys, risk limits, financial/legal review, and independent security audit |
