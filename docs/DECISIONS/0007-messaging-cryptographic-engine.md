...
WetDrool requires asynchronous one-to-one messaging with device keys,
...
homeserver, Signal service, or WetDrool database into the identity or
message authority.
...
binding to the Rust `matrix-sdk-crypto` engine behind a WetDrool-owned,
...
```mermaid
flowchart LR
    Identity["WetDrool identity and authorized device key"] --> Adapter["Versioned messaging adapter"]
...
- Each engine device key must be bound to a current WetDrool identity
...
WetDrool API. The adapter version pins the upstream event formats it
...
- **Treat a hosted Matrix service as the account system:** conflicts with
  WetDrool identity authority and replaceable-relay requirements.
...
WetDrool authorization before and after sensitive operations, revocation/rotation,
