...
use crate::{
    constants::{
        ACCOUNT_VERSION, CONFIG_SEED, IDENTITY_SEED, MANIFEST_HASH_BYTES, PDA_PREFIX, PDA_VERSION,
        POST_SEED, PROTOCOL_VERSION, TOMBSTONE_SEED,
    },
    errors::SocialProtocolError,
    events::PostTombstoned,
    state::{Identity, PostReference, ProtocolConfig, Tombstone, TombstoneReason},
    validation::{checked_increment, checked_next_sequence},
};

#[derive(AnchorSerialize, AnchorDeserialize, Clone)]
pub struct TombstonePostArgs {
...
pub fn handle_tombstone_post(ctx: Context<TombstonePost>, args: TombstonePostArgs) -> Result<()> {
    require!(
